...
@AgentRegistry.register(["twdm_agent"])
class TWDMStrategyAgent(LLMAgent):
    ...
    def _sync_alive_from_observation(self, observation):
        """
        根据当前 observation 的完整日志重算 7 人局存活玩家列表。
        """
        n_player = None
        dead_players = set()
        ...
        for log in observation["game_log"]:
            if log.event == "game_setting":
                n_player = sum(
                    cnt for cnt in log.content.values()
                ...
                for idx in log.content.get("dead_list", []):
                    if isinstance(idx, int) and idx > 0:
                        dead_players.add(idx)

            elif log.event == "end_vote":
                expelled = log.content.get("expelled", None)
                if isinstance(expelled, int) and expelled > 0:
                    dead_players.add(expelled)

        if n_player is None:
            # reset 后理论上一定会有 game_setting；缺失时按当前唯一支持的 7 人局兜底。
            n_player = 7

        if n_player != 7:
            raise ValueError(
                f"当前 TWDM Agent 只支持 7 人局，收到 {n_player} 人局。"
            )
    ...
    def format_observation(self, observation):
        self._sync_alive_from_observation(observation)
        phase = observation["phase"]
        day = int(observation["phase"].split("_")[0]) + 1
        identity = observation["identity"]
        supported_skill_identities = {"Werewolf", "Seer", "Witch", "Guard"}
        if "skill" in phase and identity not in supported_skill_identities:
            raise ValueError(
                f"Unsupported skill identity in 7p TWDM game: {identity}"
            )
        ...
        for day_i in range(1, day - 1):
            ...
            vote_info_at_day_i, _, pk_players_at_day_i = self.parse_vote_info(observation['game_log'],
                                                                              day_i)
            vote_info += vote_info_at_day_i
        if "pk" in phase or "skill" in phase:
            vote_info_last_day, _, pk_players_at_day_i = self.parse_vote_info(observation['game_log'],
                                                                              day - 1)
    ...
    def get_sys_prompt(self, observation):
        for log in observation["game_log"]:
            if log.event == 'game_setting':
                ...
                has_guard = log.content.get("Guard", 0) > 0
                has_witch = log.content.get("Witch", 0) > 0

                if total_players != 7:
                    raise ValueError(
                        f"当前 TWDM Agent 只支持 7 人局，收到 {total_players} 人局。"
                    )
                if has_witch and has_guard:
                    raise ValueError("7 人 TWDM 游戏不能同时配置 Witch 和 Guard。")
                if has_witch:
                    return CON.game_description_7p.format(
                        god_description=CON.witch_description
                    )
                if has_guard:
                    return CON.game_description_7p.format(
                        god_description=CON.guard_description
                    )
                raise ValueError("7 人 TWDM 游戏必须配置 Witch 或 Guard。")

        raise ValueError("game_setting not found in observation game_log.")

    def __api_generate(self, messages):
