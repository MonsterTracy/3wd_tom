...
ARCHITECTURES = ("gpt2block", "gru", "boe_mlp")
GPT2_CONFIG_FIELDS = (
    "vocab_size",
    "n_positions",
    "n_ctx",
...
class ToMModelConfig:
    ...
    def __post_init__(self):
        ...
        if self.architecture == "gpt2block" and self.d_model % self.num_heads:
...
class ToMModel(nn.Module):
    def __init__(self, config=None):
        ...
        if self.config.architecture == "gpt2block":
            gpt2_config = GPT2Config(
                vocab_size=1,
                n_positions=self.config.max_events,
                n_ctx=self.config.max_events,
                n_embd=d_model,
            ...
            self.sequence_model = GPT2Model(gpt2_config)
        else:
            ...
            if self.config.architecture == "gru":
                self.sequence_model = nn.GRU(
                    ...
                    dropout=(
                        self.config.dropout if self.config.num_layers > 1 else 0.0
                    ),
                    batch_first=True,
                )
            else:
    ...
    def gpt2_config_metadata(self):
        if self.config.architecture != "gpt2block":
            return None
        return {
            name: getattr(self.sequence_model.config, name)
            for name in GPT2_CONFIG_FIELDS
        }
    ...
    def _pool(self, encoded, event_mask):
        if self.config.architecture == "gpt2block":
            encoded = self.sequence_model(
                inputs_embeds=encoded,
                attention_mask=event_mask,
                use_cache=False,
                return_dict=True,
            ).last_hidden_state
            last_index = event_mask.sum(1) - 1
            return encoded[
                torch.arange(encoded.shape[0], device=encoded.device), last_index
            ]
        if self.config.architecture == "gru":
